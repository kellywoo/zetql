...
import { OrderItemSummaryModel, OrderSummaryModel } from '@/models/OrderSummary.model';
...
export interface OrderCartSubject {
	...
	add: (product: ProductModel, variantOption: Set<string>, quantity: number) => void;
	summarizeReceipt: (param?: Pick<OrderCartSubject, 'itemsGroup' | 'coupon'>) => OrderSummaryModel;
...
export const orderCartState = createSubject<OrderCartSubject>((set, get, subscribe) => {
	const changeAmount = (productId: string, variantOption: Set<string>, num: number, min: number, max?: number) => {
		try {
			...
			const newOptionVariant = productItem.variantList.map((variant) => {
				if (isSameOptionVariant(variantOption, variant.options)) {
					...
					return { options: variant.options, quantity: typeof max === 'number' ? Math.min(quantity, max) : quantity };
				} else {
					return variant;
				}
			});
	...
	const summarizeReceipt = (param?: Pick<OrderCartSubject, 'itemsGroup' | 'coupon'>) => {
		const { itemsGroup, coupon } = param || get();
		const summarizedItem: Array<OrderItemSummaryModel> = [];
		let totalProductPrice = 0;
		for (const [key, value] of itemsGroup) {
	...
	const add = (product: ProductModel, variantOption: Set<string>, quantity: number) => {
		try {
			const { productId } = product;
			const itemsGroup = get().itemsGroup;
			let productItem = itemsGroup.get(productId)!;
			if (!productItem) {
				productItem = { variantList: [{ options: variantOption, quantity }], product: product };
			} else {
				let existing = false;
