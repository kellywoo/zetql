...
import { AsideModal, AsideModalStyle } from '@ui/modals/AsideModal.tsx';
...
const ProductDetailContent = ({
  ...
  const { productInCarts, add, remove, summarizeReceipt } = useSubjectValue(
    orderCartState,
    (s) => {
      return {
        add: s.add,
  ...
  const setOrderSummary = useSubjectValue(
    modalControlSubject,
    (s) => s.setOrderSummary
  );
  ...
  const variantList = useMemo(() => {
    if (!productInCarts) {
      ...
      return productInCarts.variantList.map(({ options, quantity }) => {
        return {
          options,
          optionName: options.size ? [...options].join(', ') : '기본',
          quantity,
        };
      });
    }
  ...
  const onChange = (set: Set<string>) => {
    const sortedSet = new Set(
      product.option.filter(({ name }) => set.has(name)).map(({ name }) => name)
    );
  ...
  const placeAnOrder = () => {
    ...
    item.set(product.productId, {
      product,
      variantList: [{ options: optionSelected, quantity }],
    });
  ...
  return (
    <>
      <AsideModalStyle.Header title={'메뉴 선택'} />
      <AsideModalStyle.Container>
        <BasicInfo>
                ...
                {variantList.map(({ optionName, options, quantity }) => {
                  return (
                    <li key={optionName}>
                      ...
                      >
                        <p>{optionName}</p>
                        <span className={'fc-point fs-small'}>
                          (x&nbsp;{quantity})
                        </span>
                      </OptionSwitchButton>
...
export const ProductDetailModal = () => {
  const { productDetailId, setProductDetailId } =
    useSubjectValue(modalControlSubject);
