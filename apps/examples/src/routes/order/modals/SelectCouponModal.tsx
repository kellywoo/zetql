...
const IsReady = ({
  ...
  const { coupon: selectedCoupon, selectCoupon } =
    useSubjectValue(orderCartState);
  return (
    <Container>
      {selectedCoupon ? (
        <WithdrawCoupon>
...
const SelectCouponContent = ({ onClose }: { onClose: () => void }) => {
  const { isLoading, data } = useSubjectValue(couponQuery);
  return (
    <>
      <Wrapper>
        <Header>
        ...
        {isLoading ? (
          ...
          <IsReady coupons={data.coupons} onClose={onClose} />
        )}
      </Wrapper>
...
export const SelectCouponModal = () => {
  const { showCouponSelectModal, setShowCouponSelectModal } =
